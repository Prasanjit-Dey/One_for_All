...
class EarlyStopping:
    ...
    def save_checkpoint(self, val_loss, model, path):
        ...
        torch.save(lora.lora_state_dict(model), path + '/' + 'checkpoint.pth')
...
def visual(true, preds=None, name='./pic/test.pdf'):
    ...
    if preds is not None:
        plt.plot(preds, label='Prediction', linewidth=1, color='red')
    plt.plot(true, label='GroundTruth', linewidth=1, color='green')
    plt.legend()
...
def vali(model, vali_data, vali_loader, criterion, args, device, itr):
    total_loss = []
    if args.model == 'PatchTST' or args.model == 'DLinear' or args.model == 'TimesNet':
        model.eval()
    else:
        model.in_layer.eval()
...
def test(model, test_data, test_loader, args, device, itr,  setting):
    preds = []
    trues = []
    # mases = []
    ...
    with torch.no_grad():
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in tqdm(enumerate(test_loader)):
            ...
            #pred = scaler.inverse_transform(pred)  # Apply inverse transform
            #true = scaler.inverse_transform(true)  # Apply inverse transform
            ...
            if i % 20 == 0:
                    ...
                    #shape = input.shape
                    #input = test_data.input.squeeze(0).reshape(shape)
