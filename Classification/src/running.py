...
def setup(args):
    ...
    if not os.path.isdir(output_dir):
        logger.info(f"Root directory '{output_dir}' does not exist. Creating it...")
...
def validate(val_evaluator, tensorboard_writer, config, best_metrics, best_value, epoch):
    ...
    if condition:
        ...
        # Save predictions in a format that handles inconsistent shapes
        pred_filepath = os.path.join(config['pred_dir'], 'best_predictions.pkl')
        logger.info(f"Saving predictions to {pred_filepath}...")
        with open(pred_filepath, 'wb') as f:
            pickle.dump(per_batch, f)
